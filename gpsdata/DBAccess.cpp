...
/*
void DBAccess::GpsInsertDB( void* pData )
{
	if (NULL == pData)
	{
	...
	char strSql[1024] = {0};
	sprintf(strSql, "INSERT INTO tb_gpsdata_test(deviceid, gpsdate,insertdate, s, latitude, d, longitude, g, speed, direction) \
		                  values ('%s', '%s',now(), '%s', %f, '%s', %f,'%s', %d, %d)", 
						  pGpsData->m_strDeviceID.c_str(),
						  pGpsData->m_strDeviceDate.c_str(),
						  pGpsData->m_strS.c_str(),
						  pGpsData->m_dfLatitude,
						  pGpsData->m_strD.c_str(), 
						  pGpsData->m_dfLongitude,
						  pGpsData->m_strG.c_str(),
						  pGpsData->m_nSpeed,
...
void DBAccess::AlarmInsertDB( void* pData )
{
	if (NULL == pData)
	{
		zlog_error(g_server_cat,"pData NULL !");
	...
	sprintf(strSql, "INSERT INTO tb_gps_alarm(deviceid, gpsdate,insertdate, s, latitude, d, longitude, g, speed, direction,  \
						  ...
						  pAlarmData->m_strDeviceID.c_str(),
						  pAlarmData->m_strDeviceDate.c_str(),
						  pAlarmData->m_strS.c_str(),
						  pAlarmData->m_dfLatitude,
						  pAlarmData->m_strD.c_str(), 
						  pAlarmData->m_dfLongitude,
						  pAlarmData->m_strG.c_str(),
						  pAlarmData->m_nSpeed,
...
void DBAccess::DevStatusInsertDB( void* pData )
{
	if (NULL == pData)
	{
		zlog_error(g_server_cat,"pData NULL !");
	...
	sprintf(strSql, "update tb_dev_status set heartdate = '%s', insertdate = now(), voltage_level = %d, signal_strength = %d, \
		                  ...
		                  where deviceid = '%s'", 
						  pDevStatData->m_strDeviceDate.c_str(),
						  ...
						  pDevStatData->m_strDeviceID.c_str()
						  );
	zlog_info(g_server_cat,"Sql: %s", strSql);
	res = mysql_query(mysql, strSql);
	if (res)
...
DBAccess::DBAccess():m_MysqlConn(NULL)
{
...
bool DBAccess::initDB(std::string strIP,std::string strDbName,std::string strUser,std::string strPassword)
{

	char reconnectArg = 1;

	if(mysql_init(&m_MysqlConn) != NULL){

		//设置自动重连
		mysql_options(&m_MysqlConn, MYSQL_OPT_RECONNECT, (char *)&reconnectArg);  

		//连接数据库
		if(mysql_real_connect(&m_MysqlConn, strIP.c_str(), strUser.c_str(), strPassword.c_str(), strDbName.c_str(), 0, NULL, 0) != NULL)
		{
			zlog_info(g_server_cat,"Connect DB Success");
			return true;
		}
		else
		{

			zlog_info(g_server_cat,"Connect DB Err: %s", mysql_error(&m_MysqlConn));
			return false;
		}
...
void DBAccess::GpsInsertDB( void* pData )
{
	...
	zlog_info(g_server_cat,"Sql: %s", strSql);
	int res;
	res = mysql_query(&m_MysqlConn, strSql);
	if (res)
	{
		zlog_error(g_server_cat,"Insert Failed:%d, desc: %s", mysql_errno(&m_MysqlConn), mysql_error(&m_MysqlConn));
	}
}
